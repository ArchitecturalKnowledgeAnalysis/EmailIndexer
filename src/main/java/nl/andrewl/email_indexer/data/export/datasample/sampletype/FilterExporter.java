...
public class FilterExporter extends SampleExporter {
     ...
     *                   exceptions.
     */
    private void exportNextChunk(int page, int rank, EmailRepository emailRepo, TagRepository tagRepo,
            EmailSearcher searcher) throws Exception {
        ...
        if (rank <= this.params.getMaxResultCount() && results.hasNextPage()) {
            exportNextChunk(page++, rank, emailRepo, tagRepo, searcher);
        }
    }
}
